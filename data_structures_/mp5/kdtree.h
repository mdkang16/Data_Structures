...
class KDTree
{
  ...
  private:
     ...
     */
     vector<Point<Dim>> newvector;
     void const_help(int first, int last, int dimension, KDTreeNode* root);
     void Quickselect(int left, int right, int dimension, int middle_index);
     int Partition(int left, int right, int dimension, int pivot_index);
     void clear(KDTreeNode* subNode);
     void clear();
     void neighbor_help(int first, int last, double smaller_rad, int dimension, const Point<Dim>&query, Point<Dim>&current_best) const;
     double radius(const Point<Dim> & pt_one, const Point<Dim> & pt_two) const;
};
