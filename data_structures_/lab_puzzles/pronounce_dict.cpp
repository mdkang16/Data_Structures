    ...
    if (pronun_dict_file.is_open()) {
        while (getline(pronun_dict_file, line)) {
            ...
            if (line[0] != '#' && *line_begin != ";;;") {
                 ...
                 * (its pronunciation). */
                istream_iterator<string> str_itr = line_begin;
                dict[*str_itr] = vector<string>(++line_begin, line_end);
            }
        }
    }
...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    /* Your code goes here! */
    string str1 = word1;
    std::transform(word1.begin(), word1.end(), str1.begin(), toupper);
    string str2 = word2;
    std::transform(word2.begin(), word2.end(), str2.begin(), toupper);
    if(dict.find(str1)!=dict.end() && dict.find(str2)!=dict.end()
    && dict.find(str1)->second==dict.find(str2)->second){
      return true;
    }
