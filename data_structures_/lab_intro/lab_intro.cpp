...
PNG createSpotlight(PNG image, int centerX, int centerY) {
 for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      HSLAPixel & pixel = image.getPixel(x, y);
      int z = sqrt(((x-centerX)*(x-centerX)) + ((y-centerY)*(y-centerY)));
      if (z<160)
      {
        pixel.l = pixel.l * (1-0.005* z);
      }
      else
      {
        pixel.l = pixel.l * 0.2;
...
PNG watermark(PNG firstImage, PNG secondImage) {
  for (unsigned x = 0; (x < secondImage.width() && x < firstImage.width()); x++) {
    for (unsigned y = 0; (y < secondImage.height() && y < firstImage.height()); y++) {
      HSLAPixel & pixelf = firstImage.getPixel(x, y);
      HSLAPixel & pixels = secondImage.getPixel(x, y);

      if (pixels.l==1)
      {
        pixelf.l = (pixelf.l + 0.2 > 1) ? 1 : pixelf.l + 0.2;
      }
    }
  }
  return firstImage;
