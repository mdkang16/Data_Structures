...
class BinaryTree
{
  ...
  private:
    ...
    T find(const Node* subRoot) const;
    void printPaths(const Node* subRoot, vector<vector<T> > &paths, vector<T> &vec) const;
    int sumDistances(const Node* subRoot, int distance) const;
};
