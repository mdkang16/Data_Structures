...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
         ...
         * vector as well.
         */
         string mykey=ll.customer.substr(1,ll.customer.size()-3);
         string myURL=ll.url;
		     mykey.append(myURL);
		     if(whenVisitedTable.keyExists(mykey) && whenVisitedTable[mykey]<ll.date){
           whenVisitedTable[mykey]=ll.date;
		     }
		     else{
           whenVisitedTable.insert(mykey, ll.date);
         }
		     if(pageVisitedTable.keyExists(myURL)==false){
           uniqueURLs.push_back(myURL);
         }
		     pageVisitedTable[myURL]=true;
    }
    infile.close();
}
...
bool LogfileParser::hasVisited(const string& customer, const string& url) const
{
     ...
     */
     string mykey=customer.substr(1, customer.size()-3);
     string myURL=url;
     mykey.append(myURL);
     return whenVisitedTable.keyExists(mykey);
}
                                  ...
                                  const string& url) const
{
     ...
     */
     ...
     string myURL=url;
     mykey.append(myURL);
     return whenVisitedTable.find(mykey);
}
