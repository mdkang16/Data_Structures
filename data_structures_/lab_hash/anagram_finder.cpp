...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
    ...
    Dict<char, int> word_hash(26);
    Dict<char, int> test_hash(26);
    if(word.length() != test.length()){
      return false;
    }
    for(char i='a'; i<='z'; i++){
      word_hash[i]=test_hash[i]=0;
    }
    for(size_t i=0; i<word.length(); i++){
      word_hash[tolower(word[i])]++;
    }
   	for(size_t i=0; i<test.length(); i++){
      test_hash[tolower(test[i])]++;
    }
   	for(char i='a'; i<='z'; i++){
   		if(word_hash[i]!=test_hash[i])
   				return false;
   	}
    return true;
